...
import CentralFinanceiraView from './src/views/centralFinanceira/CentralFinanceiraView';
import { Session } from '@supabase/supabase-js';
import { registerPayment } from './src/utils/finance';

const App: React.FC = () => {
  const [session, setSession] = useState<Session | null>(null);
  const [activeTab, setActiveTab] = useState<'dashboard' | 'alerts' | 'clients' | 'transactions' | 'active_debts' | 'spc_debts' | 'backup' | 'central_financeira'>('dashboard');
  ...
  const fetchDebts = useCallback(async () => {
    ...
    if (error) {
      ...
      setDebts(data.map((d: any) => ({
        ...
        installments: d.installments?.sort((a: any, b: any) => a.installment_number - b.installment_number) || [],
  ...
  const handlePayInstallment = useCallback(async (debt: Debt, installment: any) => {
    if (!session?.user || isSaving) return;

    if (!confirm(`Confirma o pagamento da parcela ${installment.installment_number} no valor de R$ ${installment.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}?`)) return;

    setIsSaving(true);
    try {
      const { payInstallment } = await import('./src/utils/finance');
      await payInstallment(debt, installment, session.user.id);
      alert(`Parcela ${installment.installment_number} paga com sucesso!`);
      fetchDebts();
      fetchAnnualStats();
    } catch (error: any) {
      alert('Erro ao processar pagamento da parcela: ' + error.message);
    } finally {
      setIsSaving(false);
    }
  }, [fetchDebts, fetchAnnualStats, session, isSaving]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background-light dark:bg-background-dark">
      ...
      <main className="flex-1 overflow-y-auto relative scrollbar-hide pb-16 md:pb-0">
        <Header
          activeTab={activeTab}
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto animate-fadeIn">
          ...
          {activeTab === 'central_financeira' && (
            <CentralFinanceiraView />
          )}
      ...
      <nav className="bottom-nav md:hidden">
        <button onClick={() => setActiveTab('dashboard')} className={`bottom-nav-item ${activeTab === 'dashboard' ? 'active' : ''}`}>
          <span className="material-symbols-outlined text-xl">grid_view</span>
          <span>Painel</span>
        </button>
        <button onClick={() => setActiveTab('active_debts')} className={`bottom-nav-item ${activeTab === 'active_debts' ? 'active' : ''}`}>
          <span className="material-symbols-outlined text-xl">list_alt</span>
          <span>Débitos</span>
        </button>
        <button onClick={() => setShowForm(true)} className="bottom-nav-item">
          <div className="w-10 h-10 bg-primary rounded-2xl flex items-center justify-center text-white shadow-lg shadow-primary/30 -mt-4">
            <span className="material-symbols-outlined text-xl">add</span>
          </div>
        </button>
        <button onClick={() => setActiveTab('transactions')} className={`bottom-nav-item ${activeTab === 'transactions' ? 'active' : ''}`}>
          <span className="material-symbols-outlined text-xl">receipt_long</span>
          <span>Caixa</span>
        </button>
        <button onClick={() => setActiveTab('alerts')} className={`bottom-nav-item ${activeTab === 'alerts' ? 'active' : ''}`}>
          <span className="material-symbols-outlined text-xl">notifications</span>
          {stats.overdueCount > 0 && <span className="absolute -top-0.5 right-1 w-2 h-2 bg-danger rounded-full"></span>}
          <span>Alertas</span>
        </button>
      </nav>
