...
const AlertsView: React.FC<AlertsViewProps> = ({
    ...
    const selectedCustomerDebts = selectedCustomerCode ? groupedDebts[selectedCustomerCode] || [] : [];
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-8 animate-in slide-in-from-bottom-4 duration-500">
            <div className={`flex-1 space-y-4 ${selectedCustomerCode ? 'hidden lg:block' : 'block'}`}>
                <h2 className="text-sm font-black uppercase text-gray-400 tracking-widest px-2 mb-4">Fila de Cobrança</h2>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="space-y-3 max-h-[500px] overflow-y-auto scrollbar-hide pr-1">
                                {selectedCustomerDebts.map(debt => (
                                    <DebtItemRow key={debt.id} debt={debt} onPay={handlePay} onDelete={handleDelete} onEdit={onEdit} />
                                ))}
                            </div>
                            <div className="pt-4 border-t border-gray-100 dark:border-slate-700 flex gap-2">
                                ...
                                <button className="flex-1 py-3 bg-primary text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
